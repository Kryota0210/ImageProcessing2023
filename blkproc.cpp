...
//-------ENCODE
  for(size_t c = 0; c < buf.size(); ++c){
    ...
    buf[c] -= 128;                        //全画素の値から128引く(DCレベルシフト)
...
void mozaic(cv::Mat &in){       //8x8　ブロック内の各行を各行の先頭の値で塗りつぶす処理。
...
void line_mozaic(cv::Mat &in){       //8x8　ブロック内の各行を各行の先頭の値で塗りつぶす処理。
  if(in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1){
    printf("input for mozaic() shall be 8x8 and monochrome.\n");
    exit(EXIT_FAILURE);
  }

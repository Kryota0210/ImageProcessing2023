...
//ラムダ式
auto filter3x3 = [](uint8_t *p, const int stride, float *filter, const int N2){
    int idx_filter = 0;
    float val = 0.0F;
  ...
  // 画像の表示
  cv::imshow("Original", img);              //元の出力画像
  cv::imshow("Filtered", img_out);          //設定したサイズの平均値フィルタをかけた(ぼかした)画像出力
  // キー入力を待つ
  cv::waitKey();
  // 全てのウインドウを破棄
